...
const {todos,populteTodos,populteUsers,users} = require('./seed/seed');


beforeEach(populteTodos);
...
describe('DELETE todos/:id',()=>{
  ...
  it('should not remove a todo that user not own', (done) => {
...
describe('PATCH /todos/:id', () => {
  it('should update todo' , (done) => {
    var hexId = todos[0]._id.toHexString();
    var text = 'this would came from test';
...
describe('POST /users' , () => {
  ...
  it('should return validation errors if request invaild', (done) => {
    request(app)
      .post('/users')
      .send({email:'yossi##',password:'123baa'})
      .expect(400)
      .end(done);
  });
    ...
    it('should reject invalid login', (done) => {
      request(app)
        ...
        .end((err,res) => {
          ...
          User.findById(users[1]._id).then((user) => {
            expect(user.tokens.lentgh).toBe(1);
            done();
          }).catch((e) => done());
        });
    });
